...
--CÂU 3 LAB2: câu 2 đến câu 5 phần II
-- Câu 2 Phần II : Tạo quan hệ SANPHAM1 chứa toàn bộ dữ liệu của quan hệ SANPHAM. Tạo quan hệ KHACHHANG1 chứa toàn bộ dữ liệu của quan hệ KHACHHANG.
SELECT * INTO SANPHAM1 FROM SANPHAM
SELECT * INTO KHACHHANG1 FROM KHACHHANG
-- Câu 3 Phần II: Cập nhật giá tăng 5% đối với những sản phẩm do “Thai Lan” sản xuất (cho quan hệ SANPHAM1)
UPDATE SANPHAM1
SET GIA = GIA * 1.05
...
--CÂU 5 LAB 2: Câu 1 đến câu 11 phần III
...
FROM 
	SANPHAM 
WHERE 
	NUOCSX='Trung Quoc' AND GIA>=30000 AND GIA<=40000
...
FROM 
	SANPHAM 
WHERE 
	(NUOCSX='Trung Quoc' OR NUOCSX='Thai Lan') AND (GIA>=30000 AND GIA<=40000)
...
FROM 
	HOADON 
WHERE 
	NGHD ='2007-1-1' OR NGHD='2007-1-2'
...
FROM 
	HOADON 
WHERE 
	NGHD ='2007-1-1' ORDER BY NGHD ASC, TRIGIA DESC;
...
FROM 
	KHACHHANG
WHERE
	MAKH IN (SELECT MAKH FROM HOADON WHERE NGHD ='2007-1-1')
...
FROM
	HOADON
WHERE 
	NGHD = '2006-10-28' AND MANV IN (SELECT MANV FROM NHANVIEN WHERE HOTEN = 'Nguyen Van B')
...
FROM
	SANPHAM
WHERE
	MASP IN (SELECT MASP FROM CTHD WHERE SOHD IN (SELECT SOHD FROM HOADON WHERE MAKH IN (SELECT MAKH FROM KHACHHANG WHERE HOTEN='Nguyen Van A') AND MONTH(NGHD) = 10 AND YEAR(NGHD) = 2006));
--Câu 11: Tìm các số hóa đơn đã mua sản phẩm có mã số “BB01” hoặc “BB02”
SELECT
	DISTINCT HOADON.SOHD
FROM
	HOADON,CTHD
WHERE 
